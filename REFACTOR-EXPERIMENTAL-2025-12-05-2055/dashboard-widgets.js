/**
 * @fileoverview Dashboard widget component library
 * @module ui/DashboardWidgets
 * @status @canonical
 * 
 * PURPOSE:
 *   Provides the individual widget components for the Coach Mission Control dashboard.
 *   Each widget extends DashboardWidget base class and handles its own rendering,
 *   data fetching, and user interactions.
 * 
 * DEPENDENCIES:
 *   - window.dashboardManager (DashboardManager) - Data aggregation and caching
 *   - window.clientManager (ClientManager) - Client data access
 *   - window.DateHelpers - Date formatting utilities
 * 
 * WIDGET CLASSES:
 *   - DashboardWidget - Base class with loading states and debounced refresh
 *   - FlightPlanWidget - Daily priority zones (Red/Purple/Yellow/Green)
 *   - JourneyRadarWidget - Client journey stage visualization
 *   - QuickActionsWidget - Common action shortcuts
 *   - ChangesTodayWidget - Recent activity feed
 *   - GapsWidget - Missing/overdue items tracker
 *   - PipelineWidget - Discharge pipeline view
 *   - ProgramSpotlightWidget - Featured program display
 *   - HouseHealthWidget - House occupancy and health metrics
 *   - DashboardWidgets - Widget orchestration manager
 * 
 * USED BY:
 *   - CareConnect-Pro.html (dashboard initialization)
 *   - dashboard-manager.js (widget coordination)
 * 
 * EXPORTS TO WINDOW:
 *   - Class exports only (instances created in HTML)
 */
...
class FlightPlanWidget extends DashboardWidget {
    ...
    renderPriorityItem(item) {
        ...
        // Always show "Complete Task" for all action buttons
        const actionText = 'Complete Task';
