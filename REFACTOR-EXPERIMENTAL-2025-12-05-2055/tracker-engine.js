/**
 * @fileoverview TrackerEngine - Intelligence layer for CM Tracker (Legacy)
 * @module legacy/TrackerEngine
 * @status @deprecated - DO NOT ADD NEW FEATURES
 * 
 * ╔═══════════════════════════════════════════════════════════════════════════╗
 * ║  LEGACY MODULE - SCHEDULED FOR REMOVAL IN v14                             ║
 * ║  All new task-related functionality should use TaskService + TaskSchema   ║
 * ╚═══════════════════════════════════════════════════════════════════════════╝
 * 
 * PURPOSE:
 *   Calculates completion scores, identifies gaps, generates priority tasks,
 *   and provides compliance metrics for the dashboard and CM Tracker tab.
 *   This predates the modern TaskService/TaskSchema pattern.
 * 
 * DEPENDENCIES:
 *   - window.DateHelpers (optional) - Uses when available for date calculations
 *   - Client objects with milestone boolean fields (needsAssessment, etc.)
 * 
 * EXPORTS TO WINDOW:
 *   - window.trackerEngine - Singleton instance (created on first use)
 *   - window.TrackerEngine - Class constructor
 *   - window.trackerEngineWarned - Dev warning flag
 * 
 * STILL USED BY:
 *   - CareConnect-Pro.html (dashboard widgets, compliance panels)
 *   - morning-review-dashboard.js (daily review calculations)
 *   - tracker-bulk-update.js (quick update modal)
 *   - tracker-timeline.js (visual timeline)
 *   - tracker-aftercare-cascade.js (aftercare workflows)
 *   - document-generator.js (completion percentage display)
 *   - js/widgets/compliance-widget.js (dashboard compliance widget)
 * 
 * MIGRATION TARGET:
 *   - TaskService (js/task-service.js) for task state management
 *   - TaskSchema (configs/task-schema.js) for requirement definitions
 * 
 * KEY METHODS:
 *   - getClientRequirements(client) - Returns array of requirement statuses
 *   - getCompletionScore(client) - Returns percentage completion (0-100)
 *   - getHouseCompliance(clients) - House-level compliance metrics
 *   - getOverdueItems(client) - Returns array of overdue requirements
 *   - getCriticalItems(client) - Returns critical incomplete items
 * 
 * REMOVAL BLOCKERS:
 *   - 6+ modules still import this
 *   - Dashboard compliance widget depends on getHouseCompliance()
 *   - No equivalent method in TaskService for house-level rollups
 * 
 * @deprecated Use TaskService for new features. This module will be removed in v14.
 */

class TrackerEngine {
    constructor() {
        // Development warning for legacy usage
        if (window.location.hostname === 'localhost' && !window.trackerEngineWarned) {
            // Suppressed for beta to reduce console noise, but kept for dev awareness
            // console.warn('⚠️ TrackerEngine is deprecated. Use TaskService for new features.');
            window.trackerEngineWarned = true;
        }
    ...
    calculateDaysToDischarge(client) {
        if (client.dischargeDate && window.DateHelpers?.calculateDaysUntil) {
    ...
    addDays(date, days) {
        // Delegate to DateHelpers if available
        if (window.DateHelpers?.addDays) {
            return window.DateHelpers.addDays(date, days);
        }
