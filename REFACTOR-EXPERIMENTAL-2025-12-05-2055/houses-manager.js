/**
 * @fileoverview Residential house/facility management
 * @module managers/HousesManager
 * @status @canonical
 * 
 * PURPOSE:
 *   Manages the residential houses (NEST, Cove, Hedge, Meridian, Banyan) where
 *   clients stay during treatment. Handles house definitions, capacity tracking,
 *   and sub-unit management (e.g., NEST has Preserve and Prosperity wings).
 * 
 * DEPENDENCIES:
 *   - window.dbManager (IndexedDBManager) - Required for persistence
 * 
 * EXPORTS TO WINDOW:
 *   - window.HousesManager - Class constructor
 *   - window.housesManager - Singleton instance (created in CM Tracker init)
 * 
 * USED BY:
 *   - CareConnect-Pro.html (CM Tracker initialization, line ~15867)
 *   - dashboard-manager.js (house health calculations)
 *   - client-manager.js (house assignment validation)
 * 
 * HOUSE DEFINITIONS:
 *   - house_nest: NEST (capacity 20, neurodivergent program)
 *   - house_cove: Cove (capacity 15, units A/B)
 *   - house_hedge: Hedge (capacity 12)
 *   - house_meridian: Meridian (capacity 10)
 *   - house_banyan: Banyan (capacity 10)
