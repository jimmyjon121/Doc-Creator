/**
 * @fileoverview Visual aftercare options cascade and decision flow
 * @module legacy/AftercareCascade
 * @status @legacy - DO NOT ADD NEW FEATURES
 * @deprecated Will migrate to TaskService-based workflow in v14
 * 
 * PURPOSE:
 *   Renders a visual cascade/flow diagram showing aftercare program options
 *   and family decisions. Allows tracking of option status (contacted, confirmed,
 *   declined) through a step-by-step visual interface.
 * 
 * DEPENDENCIES:
 *   - window.clientManager (ClientManager) - Client data
 *   - window.aftercareManager (AftercareManager) - Aftercare options
 *   - window.outcomeTrackingModal (OutcomeTrackingModal) - Outcome capture
 *   - window.programsData - Program definitions for option lookup
 * 
 * EXPORTS TO WINDOW:
 *   - window.aftercareCascade - Singleton instance
 * 
 * MIGRATION TARGET:
 *   TaskService-based aftercare workflow
 *   Should integrate with TaskSchema aftercare tasks
 * 
 * REMOVAL BLOCKERS:
 *   - Client profile Aftercare tab depends on this
 *   - Outcome tracking flow initiated from here
 * 
 * STILL USED BY:
 *   - client-profile-manager.js (Aftercare tab)
 *   - CareConnect-Pro.html (aftercare button clicks)
 */
