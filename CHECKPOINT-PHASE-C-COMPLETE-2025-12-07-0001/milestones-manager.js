/**
 * @fileoverview Client milestone and requirement tracking
 * @module managers/MilestonesManager
 * @status @canonical - Bridges legacy milestone format to TaskSchema
 * 
 * PURPOSE:
 *   Tracks completion of client milestones (Needs Assessment, Health & Physical,
 *   Aftercare Thread, etc.). Provides legacy milestone API while TaskService
 *   handles the canonical task state. Used for milestone timeline visualization.
 * 
 * DEPENDENCIES:
 *   - window.dbManager (IndexedDBManager) - Persistence
 * 
 * EXPORTS TO WINDOW:
 *   - window.MilestonesManager - Class constructor
 *   - window.milestonesManager - Singleton instance
 * 
 * RELATIONSHIP TO TaskSchema:
 *   This manager predates TaskSchema. The milestone types defined here map
 *   to TaskSchema tasks via the TASK_TO_MILESTONE_MAP in js/task-service.js.
 *   New code should prefer TaskService; this manager remains for backward
 *   compatibility and timeline visualization.
 * 
 * MILESTONE TYPES:
 *   - needs_assessment, health_physical (48-hour requirements)
 *   - aftercare_thread, options_doc, discharge_packet (aftercare)
 *   - gad_assessment, phq_assessment (clinical)
 * 
 * USED BY:
 *   - tracker-timeline.js (timeline visualization)
 *   - client-profile-manager.js (milestone display)
 *   - CareConnect-Pro.html (CM Tracker initialization)
 */
