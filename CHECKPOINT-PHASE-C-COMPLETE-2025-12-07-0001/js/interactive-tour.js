...
(function() {
    ...
    async function init() {
        ...
        // Check user-specific keys ONLY for onboarding - no global fallback
        // This ensures each new user goes through the video before the tour
        ...
        console.log(`[Tour] Checking user state for ${username}: Agreement=${agreementAccepted}, Onboarding=${onboardingComplete}, Profile=${profileComplete}`);
        ...
        // Check if onboarding (video) is incomplete - defer to first-login-flow
        // The video should play BEFORE the tour starts
        if (!onboardingComplete) {
            console.log('[Tour] Onboarding video not complete, deferring to first-login-flow.js');
            console.log('[Tour] Profile:', profileComplete, 'Onboarding:', onboardingComplete);
            // Don't auto-start - let first-login-flow.js trigger the tour when ready (after video)
            console.log('%c[Interactive Tour v3] Ready (waiting for first-login flow - video first)', 'background: #F59E0B; color: black; padding: 4px 12px; border-radius: 4px;');
            return;
        }
