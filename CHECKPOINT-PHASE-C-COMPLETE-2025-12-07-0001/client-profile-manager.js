/**
 * @fileoverview Client profile modal management
 * @module ui/ClientProfileManager
 * @status @canonical
 * 
 * PURPOSE:
 *   Controls the main Client Profile Modal (the "Chart"). Handles tab switching,
 *   data loading, and user interactions for viewing and editing a single client's
 *   information. This is the primary interface for clinical coaches to manage
 *   individual client details.
 * 
 * DEPENDENCIES:
 *   - window.clientManager (ClientManager) - Client data CRUD
 *   - window.TaskSchema - Task definitions
 *   - window.ClientTaskService - Task state management
 *   - window.DateHelpers - Date formatting
 *   - window.milestonesManager (optional) - Timeline data
 *   - window.aftercareManager (optional) - Aftercare options
 * 
 * EXPORTS TO WINDOW:
 *   - window.clientProfileManager - Singleton instance
 *   - window.viewClientDetails(clientOrId) - Function to open profile modal
 * 
 * MODAL TABS:
 *   - Tracking: Milestones & Tasks checklist
 *   - Assessments: Clinical scores (GAD-7, PHQ-9)
 *   - Aftercare: Program options & planning
 *   - Team: Care team assignments
 *   - Timeline: Visual journey history
 *   - Notes: Clinical notes
 * 
 * USED BY:
 *   - CareConnect-Pro.html (client card clicks)
 *   - cm-tracker.js (grid view client clicks)
 *   - dashboard-widgets.js (priority zone client clicks)
 */
...
class ClientProfileManager {
    ...
    calculateDaysInCare(client) {
        if (window.DateHelpers?.calculateDaysInCare) {
            return window.DateHelpers.calculateDaysInCare(client.admissionDate, client.dischargeDate);
        }
    ...
    calculateMetrics(client) {
        ...
        return {
            ...
            total,
            completion,
            startDateLabel: window.DateHelpers?.formatDate
                ? window.DateHelpers.formatDate(client.admissionDate)
                : (client.admissionDate ? new Date(client.admissionDate).toLocaleDateString() : 'Not admitted'),
...
window.viewClientDetails = function(clientOrId) {
    try {
        ...
        console.error('Failed to open client profile', error);
    }
};
