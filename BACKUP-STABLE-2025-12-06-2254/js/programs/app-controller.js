...
function createUniversalCard(program) {
  ...
  function toggleAddDropdown() {
    // Smart add: check if program is at-home type first
    if (!state.selectedProgramId) return;
    ...
    // For Aftercare Options, check if this is an at-home type program (IOP/PHP/OP)
    if (docType === 'aftercare-options' && isAtHomeProgramType(program)) {
    ...
    // Regular program - add directly to primary recommendations
    if (docType === 'aftercare-options') {
      window.ccDocumentModel?.addProgram('simple', program.id);
    } else {
      // For Aftercare Plan, show the phase dropdown
    ...
    updateBuilderUI();
    closeProfile();
    window.showNotification?.('Program added to document', 'success');
  }
  ...
  /**
   * Get program by ID (checks both database and custom programs in draft)
   */
  function getProgramById(id) {
    // Check if it's a custom program
    if (id.startsWith('custom_')) {
      const draft = window.ccDocumentModel?.getCurrentDraft();
      return draft?.customPrograms?.[id] || null;
    }
    // Look up in database
    return window.ccPrograms?.byId(id) || null;
  }
  ...
  function updateSimpleListUI(programIds) {
    ...
    if (programIds.length === 0) {
      ...
      container.innerHTML = programIds.map(id => {
        ...
        const locationText = p.location || (p.city ? `${p.city}, ${p.state || ''}` : 'Virtual');
        return `
          <div class="builder-item ${isCustom ? 'builder-item--custom' : ''}" data-program-id="${id}" draggable="true">
            <span class="builder-item__drag">⋮⋮</span>
            <div class="builder-item__content">
              <div class="builder-item__name">${isCustom ? '✏️ ' : ''}${p.name}</div>
              <div class="builder-item__location">${locationText}</div>
            </div>
  ...
  /**
   * Show modal to add a custom program not in the database
   * @param {string} phase - 'simple' or 'atHome'
   */
  function showAddCustomProgram(phase) {
    const overlay = document.createElement('div');
    overlay.className = 'custom-program-overlay';
    overlay.innerHTML = `
      <div class="custom-program-modal">
        <div class="custom-program-modal__header">
          <span class="custom-program-modal__title">
            ${phase === 'atHome' ? '🏠' : '🏥'} Add Custom Program
          </span>
          <button type="button" class="custom-program-modal__close" data-action="close">✕</button>
        </div>
        <div class="custom-program-modal__body">
          <form class="custom-program-form" id="customProgramForm">
            <div class="custom-program-form__group">
              <label class="custom-program-form__label custom-program-form__label--required">Program Name</label>
              <input type="text" name="name" class="custom-program-form__input" placeholder="e.g., ABC Therapy Center" required>
            </div>
            
            <div class="custom-program-form__row">
              <div class="custom-program-form__group">
                <label class="custom-program-form__label">Level of Care</label>
                <select name="loc" class="custom-program-form__select">
                  ...
                  <option value="RTC">Residential Treatment</option>
                  <option value="TBS">Therapeutic Boarding School</option>
              ...
              <div class="custom-program-form__group">
                <label class="custom-program-form__label">Location</label>
                <input type="text" name="location" class="custom-program-form__input" placeholder="City, State">
              </div>
            </div>
            
            <div class="custom-program-form__group">
              <label class="custom-program-form__label">Services / Specialties</label>
              <textarea name="services" class="custom-program-form__textarea" placeholder="e.g., Individual therapy, family therapy, DBT, trauma-focused care..."></textarea>
            </div>
            
            <div class="custom-program-form__row">
              <div class="custom-program-form__group">
                <label class="custom-program-form__label">Phone</label>
                <input type="tel" name="phone" class="custom-program-form__input" placeholder="(555) 123-4567">
              </div>
              <div class="custom-program-form__group">
                <label class="custom-program-form__label">Website</label>
                <input type="url" name="website" class="custom-program-form__input" placeholder="https://...">
              </div>
            </div>
            
            <div class="custom-program-form__group">
              <label class="custom-program-form__label">Additional Notes</label>
              <textarea name="notes" class="custom-program-form__textarea" rows="2" placeholder="Any additional information..."></textarea>
            </div>
          </form>
        </div>
        <div class="custom-program-modal__footer">
          <button type="button" class="custom-program-btn custom-program-btn--cancel" data-action="close">Cancel</button>
          <button type="button" class="custom-program-btn custom-program-btn--add" data-action="add">Add to Document</button>
        </div>
      </div>
    `;
    ...
    // Focus first input
    setTimeout(() => overlay.querySelector('input[name="name"]')?.focus(), 100);
    ...
    overlay.addEventListener('click', (e) => {
      ...
      if (action === 'close' || e.target === overlay) {
        ...
        const customProgram = {
          ...
          name: name,
          isCustom: true,
          primaryLOC: formData.get('loc') || '',
          city: formData.get('location')?.split(',')[0]?.trim() || '',
          state: formData.get('location')?.split(',')[1]?.trim() || '',
        ...
        // Add to draft
        addCustomProgramToDraft(phase, customProgram);
        overlay.remove();
        updateBuilderUI();
        window.showNotification?.('Custom program added', 'success');
      }
    });
  ...
  /**
   * Add custom program to draft and store it
   */
  function addCustomProgramToDraft(phase, customProgram) {
    const draft = window.ccDocumentModel?.getCurrentDraft();
    if (!draft) {
    ...
    // Store custom programs in draft
    if (!draft.customPrograms) {
      draft.customPrograms = {};
    }
    draft.customPrograms[customProgram.id] = customProgram;
    
    // Add to phase
    window.ccDocumentModel?.addProgram(phase, customProgram.id);
  }
  
  // Expose to window for onclick handlers
  window.showAddCustomProgram = showAddCustomProgram;
