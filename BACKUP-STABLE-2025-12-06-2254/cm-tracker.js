/**
 * @fileoverview Client Management Tab - Grid/List View
 * @module legacy/CMTracker
 * @status @legacy - DO NOT ADD NEW FEATURES
 * @deprecated Scheduled for replacement with ClientsView in v14
 * 
 * PURPOSE:
 *   Renders the "Clients" tab grid view showing clients grouped by house.
 *   This is the legacy view; the modern flow uses ClientManager + ClientProfileManager.
 *   Retained for beta to avoid breaking historical demos and existing workflows.
 * 
 * DEPENDENCIES:
 *   - window.clientManager (ClientManager) - Client data queries
 *   - window.housesManager (HousesManager) - House definitions
 *   - window.HousesManager - Class for deferred initialization
 *   - window.switchTab - Tab navigation (defined in CareConnect-Pro.html)
 * 
 * EXPORTS TO WINDOW:
 *   - window.initializeCMTracker - Initialize clients tab
 *   - window.refreshClientsList - Alias for initializeCMTracker
 *   - window.refreshCMTracker - Alias for initializeCMTracker
 * 
 * MIGRATION TARGET:
 *   Future ClientsView component (Phase 5 of refactor plan)
 *   New implementation should use TaskService for task status
 * 
 * REMOVAL BLOCKERS:
 *   - Clients tab depends on this for grid rendering
 *   - Demo data refresh calls initializeCMTracker
 *   - Export functionality (cm-tracker-export.js) relies on this view
 * 
 * STILL USED BY:
 *   - CareConnect-Pro.html (line ~168, Clients tab switching)
 *   - js/demo-data.js (refresh after data generation)
    ...
    try {
        ...
        if (unassigned.length > 0) {
            ...
            unassigned.forEach(client => {
                const referralDate = window.DateHelpers?.formatDate 
                    ? window.DateHelpers.formatDate(client.referralDate) 
                    : (client.referralDate ? new Date(client.referralDate).toLocaleDateString() : 'N/A');
                const intakeDate = window.DateHelpers?.formatDate 
                    ? window.DateHelpers.formatDate(client.intakeScheduledDate)
                    : (client.intakeScheduledDate ? new Date(client.intakeScheduledDate).toLocaleDateString() : 'Not scheduled');
