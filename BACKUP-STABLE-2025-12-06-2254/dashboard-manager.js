 ...
 * 
 * PURPOSE: Orchestrates the main dashboard view. Aggregates data from ClientManager,
 * TaskService, and IndexedDB to calculate priorities, house health, and journey metrics.
 * 
 * SCOPE: Main Dashboard Tab
 * STATUS: @canonical
 * 
 * RESPONSIBILITIES:
 * - Data aggregation & caching (1-minute TTL)
 * - Priority scoring logic
 * - State management for dashboard filters (Journey Stage, House)
...
class DashboardManager {
    ...
    getCurrentCoach() {
        // Get from session or localStorage (check localStorage first for persistence)
        const userRole = localStorage.getItem('userRole') || sessionStorage.getItem('userRole') || 'Coach';
        ...
        // Log for debugging admin recognition issues
        if (isAdmin && !this._adminLoggedOnce) {
            console.log('🔐 Admin user detected:', { initials, role: userRole });
            this._adminLoggedOnce = true;
        }
    ...
    async loadCriticalAlerts() {
        try {
            ...
            for (const client of clients) {
                ...
                // Discharge prep checks (check both expected and actual discharge date)
                const dcDate = client.dischargeDate || client.expectedDischargeDate;
                if (dcDate) {
                    const daysUntilDischarge = this.calculateDaysUntil(dcDate);
    ...
    async loadJourneyData() {
        try {
            ...
            for (const client of clients) {
                ...
                // Check if in discharge pipeline (has future discharge date - expected or actual)
                const dcDate = client.dischargeDate || client.expectedDischargeDate;
                if (dcDate && client.status === 'active') {
                    const daysUntilDischarge = this.calculateDaysUntil(dcDate);
                    if (daysUntilDischarge >= 0) {
                        segments.dischargePipeline.push({ ...client, daysUntilDischarge, dischargeDate: dcDate });
                        continue;
                    }
                }
    ...
    async loadRecentChanges() {
        try {
            ...
            for (const client of clients) {
                ...
                // Approaching discharge (within 3 days - check both expected and actual)
                const dcDate = client.dischargeDate || client.expectedDischargeDate;
                if (dcDate && client.status === 'active') {
                    const daysUntilDischarge = this.calculateDaysUntil(dcDate);
                    if (daysUntilDischarge >= 0 && daysUntilDischarge <= 7) {
    ...
    async detectGaps() {
        try {
            ...
            for (const client of clients) {
                ...
                // Missing discharge date after 14 days (check both expected and actual)
                if (days > 14 && !client.dischargeDate && !client.expectedDischargeDate) {
                ...
                // Missing care team assignment (need Clinical Coach OR Therapist OR Family Ambassador)
                if (!client.clinicalCoachInitials && !client.primaryTherapistInitials && !client.familyAmbassadorPrimaryInitials) {
    ...
    async loadPipelineData() {
        try {
            ...
            // Discharges: active clients with discharge date in next 14 days (check both expected and actual)
            const discharges = clients.filter(c => {
                if (c.status !== 'active') return false;
                const dcDate = c.dischargeDate || c.expectedDischargeDate;
                if (!dcDate) return false;
                const daysUntil = this.calculateDaysUntil(dcDate);
                ...
                return {
                    ...
                    daysUntilDischarge: this.calculateDaysUntil(dcDate)
                };
    ...
    /**
     * Calculate days in care using centralized DateHelpers
