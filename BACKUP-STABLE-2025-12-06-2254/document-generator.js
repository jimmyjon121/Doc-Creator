 ...
 * 
 * PURPOSE: Manages the wizard workflow for creating PDF documents (Aftercare Options,
 * Plans, Discharge Packets). Enforces clinical workflow rules (e.g., prerequisites).
 * 
 * SCOPE: Global (accessible via window.generateDocument)
 * STATUS: @canonical
 * 
 * DEPENDENCIES:
 * - jspdf / html2canvas / pdf-lib (external libs)
 * - ClientManager (for client data)
 * - TaskService (for updating completion status)
 */
                ...
                },
                {
                    ...
                    action: async () => {
                        ...
                        if (window.clientManager?.updateClient) {
                            await window.clientManager.updateClient(client.id, {
...
window.generateDocument = function(clientId = null, documentType = null) {
    try {
        ...
        console.error('Failed to launch document workflow', error);
        window.showNotification?.('Unable to start document workflow', 'error');
    }
};
