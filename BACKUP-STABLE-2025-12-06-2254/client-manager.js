/**
 * @fileoverview Client data management and CRUD operations
 * @module managers/ClientManager
 * @status @canonical
 * 
 * PURPOSE:
 *   Central manager for all client records. Handles creation, updates, deletion,
 *   and querying of client data. Enforces HIPAA compliance by storing only
 *   initials + Kipu ID (no PHI). Syncs with TaskSchema for task state.
 * 
 * DEPENDENCIES:
 *   - window.dbManager (IndexedDBManager) - Required for IndexedDB persistence
 *   - window.TaskSchema (optional) - For task state initialization
 * 
 * EXPORTS TO WINDOW:
 *   - window.ClientManager - Class constructor
 *   - window.clientManager - Singleton instance (created in CareConnect-Pro.html)
 * 
 * USED BY:
 *   - CareConnect-Pro.html (initialization)
 *   - client-profile-manager.js (profile modal)
 *   - dashboard-manager.js (data aggregation)
 *   - cm-tracker.js (legacy grid view)
 *   - js/task-service.js (task management)
 *   - js/demo-data.js (test data generation)
 * 
 * DATA NOTES:
 *   - Client records use initials (max 4 chars) + Kipu ID only
 *   - No names, SSNs, or other PHI stored in IndexedDB
 *   - Status: 'active' | 'discharged'
 *   - Task state stored in client.taskState object
 * 
 * @example
 *   const client = await window.clientManager.createClient({
 *     initials: 'JD',
 *     kipuId: '12345',
 *     houseId: 'house_nest',
...
class ClientManager {
    ...
    async createClient(clientData) {
        ...
        const client = {
            ...
            coveUnit: clientData.coveUnit || null, // 'A' or 'B' for Cove house only
            admissionDate: clientData.admissionDate || null,
            expectedDischargeDate: clientData.expectedDischargeDate || null, // Target discharge date
            dischargeDate: clientData.dischargeDate || null, // Actual discharge date
     ...
     * @param {boolean} activeOnly - when true, return only active clients
     */
    ...
    showDischargeBlockerModal(readiness, options = {}) {
        ...
        // Check if user is admin for override capability (case-insensitive)
        const currentUser = JSON.parse(localStorage.getItem('currentUser') || '{}');
        const userRole = (localStorage.getItem('userRole') || currentUser.role || '').toLowerCase();
        const isAdmin = userRole === 'admin' || currentUser.permissions?.includes('admin');
