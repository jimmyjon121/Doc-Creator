document.addEventListener('DOMContentLoaded', function() {
    ...
    copyBtn.addEventListener('click', function() {
        ...
        addActivityLog('Starting Copy to Tool process...');
        ...
        navigator.clipboard.writeText(formattedText).then(() => {
            ...
            addActivityLog('⚠️ Clipboard copy failed: ' + err.message, 'warning');
        });
        
        // Simple approach - just open the URL
        addActivityLog('Opening Doc Creator app...');
        
        // First check if Doc Creator is already open
        chrome.tabs.query({}, function(tabs) {
            let docCreatorTab = null;
            
            // Look for an existing Doc Creator tab
            for (let tab of tabs) {
                if (tab.url && (
                    tab.url.includes('AppsCode-DeluxeCMS.html') ||
                    tab.url.includes('CareConnect-Pro') ||
            ...
            if (docCreatorTab) {
                // Focus existing tab and send data
                addActivityLog('Found existing Doc Creator tab');
                chrome.tabs.update(docCreatorTab.id, {active: true}, function() {
                    handleTabOpened(docCreatorTab);
                });
            } else {
                // Open new tab with extension flag
                const primaryUrl = 'http://localhost:8000/AppsCode-DeluxeCMS.html?fromExtension=true&autoOpen=true';
                
                addActivityLog('Opening new Doc Creator tab...');
                chrome.tabs.create({ url: primaryUrl }, function(tab) {
                    if (chrome.runtime.lastError) {
                        addActivityLog('❌ Failed to open: ' + chrome.runtime.lastError.message, 'error');
                        
                        // Try file URL as fallback
                        const fileUrl = 'file:///Users/christophermolina/Cursor%20DOCCREATION%20Tool%20/Doc-Creator/Doc-Creator/AppsCode-DeluxeCMS.html?fromExtension=true&autoOpen=true';
                        addActivityLog('Trying direct file URL...');
                        
                        chrome.tabs.create({ url: fileUrl }, function(tab2) {
                            if (chrome.runtime.lastError) {
                                addActivityLog('❌ File URL also failed', 'error');
                                showStatus('Could not open Doc Creator. Data is in clipboard!', 'warning', '📋');
                                copyBtn.disabled = false;
                            } else {
                                addActivityLog('✅ Opened via file URL', 'success');
                                handleTabOpened(tab2);
                            }
                        });
                    } else {
                        addActivityLog('✅ Tab opened successfully', 'success');
                        handleTabOpened(tab);
                    }
                });
            }
    ...
    // Handle successful tab opening
    function handleTabOpened(tab) {
        addActivityLog(`Tab ID: ${tab.id} opened`);
        
        // Focus the tab
        chrome.tabs.update(tab.id, {active: true});
        
        // Check if it's a new tab (needs more time to load) or existing
        const isNewTab = tab.url && tab.url.includes('fromExtension=true');
        const waitTime = isNewTab ? 3500 : 1500;
        
        // Wait for app to load then send data
        setTimeout(() => {
            addActivityLog('Sending program data to Doc Creator...');
    ...
    function openDocCreator() {
        ...
        const urls = [
            'http://localhost:8000/FINAL-ENHANCED-v3.0/CareConnect-Pro-v3.0-ENHANCED.html',
            'http://localhost:8000/CareConnect-Pro-v3.0-ENHANCED.html',
            'http://localhost:8000/AppsCode-DeluxeCMS.html',
            'http://127.0.0.1:8000/FINAL-ENHANCED-v3.0/CareConnect-Pro-v3.0-ENHANCED.html',
            'http://127.0.0.1:8000/AppsCode-DeluxeCMS.html',
            'file:///Users/christophermolina/Cursor%20DOCCREATION%20Tool%20/Doc-Creator/Doc-Creator/FINAL-ENHANCED-v3.0/CareConnect-Pro-v3.0-ENHANCED.html'
        ];
        ...
        function tryNextUrl() {
            ...
            chrome.tabs.create({ url: url }, function(tab) {
                if (chrome.runtime.lastError) {
                    ...
                    addActivityLog(`Opened: ${url.substring(0, 50)}...`);
                    ...
                    setTimeout(() => {
                        addActivityLog('Sending data to Doc Creator...');
                        sendDataToTab(tab.id);
                    }, 3000);
                }
            });
