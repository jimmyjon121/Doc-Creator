...
const OnboardingController = (function() {
    ...
    function setupEventListeners() {
        console.log('[OnboardingController] Setting up event listeners...');
        ...
        appEvents.forEach(eventName => {
            console.log('[OnboardingController] Listening for:', eventName);
            const cleanup = OnboardingEvents.on(eventName, async (detail) => {
                console.log('[OnboardingController] Event received:', eventName, detail);
                const checklistItemId = OnboardingEvents.getChecklistItemForEvent(eventName);
                console.log('[OnboardingController] Mapped to checklist item:', checklistItemId);
                
                if (checklistItemId && state && !state.checklist[checklistItemId]) {
                    console.log('[OnboardingController] Marking item complete:', checklistItemId);
                    ...
                    // Update checklist UI
                    if (checklist) {
                        console.log('[OnboardingController] Updating checklist UI');
                        checklist.markComplete(checklistItemId);
                    } else {
                        console.warn('[OnboardingController] Checklist not available for UI update');
                    }
                    ...
                    console.log('[OnboardingController] Item already complete or state missing:', {
                        checklistItemId,
                        hasState: !!state,
                        currentValue: state?.checklist?.[checklistItemId]
                    });
                }
            });
            eventCleanups.push(cleanup);
        });
        console.log('[OnboardingController] Event listeners set up');
    }
...
if (typeof window !== 'undefined') {
    ...
    window.onboardingManager = OnboardingController; // Alias for CareConnect-Pro initialization
}
