...
(function() {
    ...
    function loadCSS() {
        ...
        // First Login CSS
        const firstLoginCSS = document.createElement('link');
        firstLoginCSS.rel = 'stylesheet';
        firstLoginCSS.href = 'css/first-login.css';
        document.head.appendChild(firstLoginCSS);
    ...
    function loadScript(src) {
        return new Promise((resolve, reject) => {
            ...
            script.onerror = (err) => {
                console.error('[Onboarding Loader] Failed to load:', src, err);
                reject(err);
            };
    ...
    async function loadOnboardingScripts() {
        const scripts = [
            // First Login Flow (must be early to catch login event)
